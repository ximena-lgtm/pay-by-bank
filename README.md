...
**La API es un solo archivo con la ruta reconstruida a mano.** `vercel.json` enruta `/api/:path*` a `/api/index` pasando el resto en el parámetro `__p`, y el handler lo vuelve a componer antes de delegar en Express. Se intentó con nombres de archivo dinámicos —`api/[[...path]].js` y `api/[...path].js`— y en ambos casos Vercel solo enrutó rutas de **un** segmento: `/api/banks` llegaba y `/api/consent/link/initiate` respondía `404 NOT_FOUND` sin tocar la función. Reconstruir la ruta desde un parámetro explícito no depende de esa interpretación. Un `api/index.js` con un rewrite de `/api/(.*)` a `/api/index` no sirve: el rewrite cambia la ruta que ve Express, que solo conoce `/api/banks`, `/api/accounts` y compañía, así que toda petición responde 404.
